    ...
    public class ImageService
    {
        ...
        public ICollection<ImageSummaryViewModel> GetImages()
        {
            ...
            foreach (Image image in images)
            {
                ...
                byte[] defaultThumbnailData;
                try
                {
                    defaultImageData = File.ReadAllBytes("C:\\Users\\Ahmad\\Source\\Repos\\Bothniabladet\\Bothniabladet\\Images\\default.jpg");
                    defaultThumbnailData = File.ReadAllBytes("C:\\Users\\Ahmad\\Source\\Repos\\Bothniabladet\\Bothniabladet\\Images\\default_thumbnail.jpg");
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Could not load default images: {ex.Message}");
                    defaultImageData = new byte[0];
                    defaultThumbnailData = new byte[0];
                }
                ...
                imageSummaryViewModels.Add(new ImageSummaryViewModel
                {
                    ...
                    ThumbnailDataString = image.Thumbnail != null ? string.Format("data:image/jpg;base64,{0}", Convert.ToBase64String(image.Thumbnail)) : string.Format("data:image/jpg;base64,{0}", Convert.ToBase64String(defaultThumbnailData)),
                    ImageDataString = image.ImageData != null ? string.Format("data:image/jpg;base64,{0}", Convert.ToBase64String(image.ImageData)) : string.Format("data:image/jpg;base64,{0}", Convert.ToBase64String(defaultImageData)),
        ...
        public ImageDetailViewModel GetImageDetail(int? id)
        {
            ImageDetailViewModel imageViewModel = _context.Images
                ...
                .Select(image => new ImageDetailViewModel
                {
                    Id = image.ImageId,
                    Name = image.ImageTitle,
                    ImageDataString = image.ImageData != null
                        ? $"data:image/jpg;base64,{Convert.ToBase64String(image.ImageData)}"
                        : string.Empty,
                    Section = image.Section.ToString(),
                    Date = image.Issue,
                    ...
                    EditedImages = image.EditedImages ?? new List<EditedImage>(), // Check if EditedImages is null and provide an empty list if it is
